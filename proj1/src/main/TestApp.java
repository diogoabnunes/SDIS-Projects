...
public class TestApp {
    ...
    public void processAccessPoint(String accessPoint) throws InvalidArgumentsException {
        if(!accessPoint.contains(":") && !Auxiliar.isInteger(accessPoint)) throw new InvalidArgumentsException("4");
        else if(!accessPoint.contains(":") && Auxiliar.isInteger(accessPoint)) this.accessPoint = accessPoint;
        else {
            String[] parts = accessPoint.split(":");
            if(parts.length != 2 || !Auxiliar.isInteger(parts[1])) throw new InvalidArgumentsException("5");
    ...
    public void processThirdArgument(String arg) throws InvalidArgumentsException {
        if(this.operation.equals("BACKUP") || this.operation.equals("RESTORE") || this.operation.equals("DELETE")) {
            if(!Auxiliar.isInteger(arg)) this.pathName = arg;
            ...
            if(!Auxiliar.isInteger(arg)) throw new InvalidArgumentsException(ConsoleColors.YELLOW + "Usage: java TestApp <peer_ap> <sub_protocol> <opnd_1> <opnd_2>" + ConsoleColors.RESET);
    ...
    public void processLastArgument(String arg) throws InvalidArgumentsException {
        if(this.operation.equals("BACKUP") && Auxiliar.isInteger(arg)) this.replicationDegree = arg;
        else throw new InvalidArgumentsException(ConsoleColors.YELLOW + "Usage: java TestApp <peer_ap> <sub_protocol> <opnd_1> <opnd_2>" + ConsoleColors.RESET);
    }
}
